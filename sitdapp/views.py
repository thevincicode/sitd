...
def expedientes(request):
    try:
        if 'EstadoLogueo' in request.COOKIES:
            value = request.COOKIES['EstadoLogueo']
            if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
                UsuarioLogueado = Trabajador.objects.get(
                    id=request.COOKIES['IdUsuario'])
                UsuarioOficina = Oficina.objects.get(
                    id=UsuarioLogueado.oficina_id)
                ...
                expediente = Expediente.objects.filter(
                    oficina=UsuarioOficina).filter(Q(estado="R") | Q(estado="C"))
                if UsuarioLogueado.admin == "S":
                    expediente = Expediente.objects.filter(
                        Q(estado="R") | Q(estado="C"))
                #expedientes=get_object_or_404(Expediente, estado=1)
                return render(request, 'expedientes.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'lista': expediente, 'oficinas': oficinas, 'proveidos': proveidos, 'organizacion': empresa})
            else:
        ...
        return render(request, 'Logueo.html', Ctx)
        ...
        if 'EstadoLogueo' in request.COOKIES:
            ...
            if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
                ...
                if request.method == "POST":
                    ...
                    try:
                        expediente_obj = Expediente(nro_exp=nro, nro_doc=codigo, folios=folios, interesado=interesado, asunto=asunto, destinatario=destinatario, f_inicio=datetime.now(), f_registro=datetime.now(
                        ), f_fint=fechap, observacion="", estado="C", tipo=tipo_obj, tupa=tupa_obj, oficina=UsuarioOficina, archivomotivo="", roficina=NombreOficina, rusuario=NombreUsuario)
                        expediente_obj.save()
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                try:
                    ...
                    ultimonro = 0
                return render(request, 'NuevoExpediente.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'tipo': tipo, 'tupas': tupas, 'tupavinculo': tupavinculo, 'organizacion': empresa, 'fechaserver': ahora, 'ultimonro': ultimonro})
            else:
...
def archivados(request):
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            proveidos = Proveido.objects.all()
            expediente = Expediente.objects.filter(
                estado="A").filter(oficina=UsuarioOficina)
            if UsuarioLogueado.admin == "S":
                expediente = Expediente.objects.filter(estado="A")
            #expedientes=get_object_or_404(Expediente, estado=1)
            return render(request, 'archivados.html', {'organizacion': empresa, 'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'lista': expediente, 'oficinas': oficinas, 'proveidos': proveidos})
        else:
            html = '<html><head><meta http-equiv="Refresh" content="3;url=/Logueo"></head><body>Error de parametro En el Login</body></html>'
            return HttpResponse(html)
    else:
...
def porrecibir(request):
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            proveidos = Proveido.objects.all()
            expediente = Expediente.objects.filter(
                estado="E").filter(oficina=UsuarioOficina)
            if UsuarioLogueado.admin == "S":
                expediente = Expediente.objects.filter(estado="E")
            #expedientes=get_object_or_404(Expediente, estado=1)
            return render(request, 'porrecibir.html', {'organizacion': empresa, 'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'lista': expediente, 'oficinas': oficinas, 'proveidos': proveidos})
        else:
            html = '<html><head><meta http-equiv="Refresh" content="3;url=/Logueo"></head><body>Error de parametro En el Login</body></html>'
            return HttpResponse(html)
    else:
...
def expediente_detalle(request, id_expediente):
    try:
        ...
        return render(request, 'expediente_detalle.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'expediente': expediente, 'historial': historial})
    except:
        NombreUsuario = ""
        NombreOficina = ""
        empresa = Organizacion.objects.get(id=1)
...
def expediente_imprimir(request, id_expediente):
    try:
        ...
        expediente = get_object_or_404(Expediente, pk=id_expediente)
        historial = ExpedienteHistorial.objects.filter(expediente=expediente)
        return render(request, 'imprimir.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'expediente': expediente})
    except:
        NombreUsuario = ""
        NombreOficina = ""
        empresa = Organizacion.objects.get(id=1)
...
def expediente_detalle_comentario(request, id_expediente, id_historial):
    try:
        ...
        return render(request, 'comentario.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'expediente': expediente, 'historial': historial})
    except:
        NombreUsuario = ""
        NombreOficina = ""
        empresa = Organizacion.objects.get(id=1)
...
def buscador(request):
    if request.GET.get("b"):
        ...
        try:
            ...
            return render(request, 'buscador.html', {'lista': buscador, 'organizacion': empresa, 'b': buscar})
        except Exception as e:
            empresa = Organizacion.objects.get(id=1)
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            if request.method == "POST":
                ...
                try:
                    if(len(id_s) > 0):
                        ...
                        for v in id_s:
                            ...
                            for o in oficina_s:
                                ...
                                print('el id de select proveido es ' +
                                      proveido_s[indexx])
                                idproveido = proveido_s[indexx]
                                print('el id proveido es ' + str(idproveido))
                                proveido = get_object_or_404(
                                    Proveido, pk=idproveido)
                                idoficina = o
                                oficina = get_object_or_404(
                                    Oficina, pk=idoficina)
                                idexpediente = v
                                expediente = get_object_or_404(
                                    Expediente, pk=idexpediente)
                                # generando un objeto de tipo Derivar y guardando
                                derivar_obj = Derivar(fecha=datetime.now(
                                ), estado="E", comentario=comentario, proveido=proveido, expediente=expediente)
                                derivar_obj.save()
                                ...
                                historial_obj = ExpedienteHistorial(f_salida=datetime.now(), o_origen=oficina_o, o_destino=oficina.nombre, f_recepcion=datetime.now(
                                ), estado="enviado", proveido=proveido.nombre, expediente=expediente, ubicacion="-", comentario=comentario, usuario=NombreUsuario)
                                historial_obj.save()
                                print("se agrego al historial")
                                # actualizando estado del expediente
                                expediente_upd = Expediente.objects.get(
                                    pk=idexpediente)
                    ...
                    html = "<html><body>error accediendo<br>" + \
                        str(e) + "</body></html>"
                    return HttpResponse(html)
            else:
                ...
                try:
                    if(len(id_s) > 0):
                        ...
                        for v in id_s:
                            ...
                            last = Derivar.objects.filter(
                                expediente=expediente_upd).order_by('fecha')
                            lon = len(last)
                            try:
                                historial_last = last[lon - 1]
                                derivar_obj = Derivar(fecha=datetime.now(
                                ), estado="A", comentario=comentario, proveido=historial_last.proveido, expediente=expediente_upd)
                                derivar_obj.save()
                                ...
                                # agregando al historial
                                historial_obj = ExpedienteHistorial(f_salida=datetime.now(), o_origen=historial_last.oficina, o_destino=UsuarioOficina.nombre, f_recepcion=datetime.now(
                                ), estado="Archivado", proveido=historial_last.proveido, expediente=expediente_upd, ubicacion="-", comentario=comentario, usuario=NombreUsuario)
                                historial_obj.save()
                            except Exception as e:
                                proveido = Proveido.objects.get(pk=1)
                                derivar_obj = Derivar(fecha=datetime.now(
                                ), estado="A", comentario=comentario, proveido=proveido, expediente=expediente_upd)
                                derivar_obj.save()
                                derivar_obj.oficina.add(expediente_upd.oficina)
                                derivar_obj.oficinao.add(UsuarioOficina)
                                derivar_obj.save()
                                ...
                                # agregando al historial
                                historial_obj = ExpedienteHistorial(f_salida=datetime.now(), o_origen=UsuarioOficina, o_destino=UsuarioOficina, f_recepcion=datetime.now(
                                ), estado="Archivado", proveido=proveido.nombre, expediente=expediente_upd, ubicacion="-", comentario=comentario, usuario=NombreUsuario)
                                historial_obj.save()
                ...
                try:
                    if(len(id_s) > 0):
                        ...
                        for v in id_s:
                            ...
                            last = ExpedienteHistorial.objects.filter(
                                expediente=expediente_upd).order_by('f_salida')
                            lon = len(last)
                            historial_last = last[lon - 1]
                            historial_obj = ExpedienteHistorial(f_salida=datetime.now(), o_origen=historial_last.o_origen, o_destino=expediente_upd.oficina.nombre, f_recepcion=datetime.now(
                            ), estado="Recibido", proveido=historial_last.proveido, expediente=expediente_upd, ubicacion="-", comentario="recibido", usuario=NombreUsuario)
                            historial_obj.save()

                            last2 = Derivar.objects.filter(
                                expediente=expediente_upd).order_by('fecha')
                            lon2 = len(last2)
                            historial_last2 = last2[lon2 - 1]
                            derivar_obj = Derivar(fecha=datetime.now(
                            ), estado="R", comentario="Recibido", proveido=historial_last2.proveido, expediente=expediente_upd)
                            derivar_obj.save()
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'tupa_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': tupa})
            else:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                if request.method == 'POST':
                    ...
                    formulario = TupaForm()
                return render(request, 'tupaform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
...
def tuparequisitos_lista(request):
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                return render(request, 'tuparequisitos_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': tupa})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'tuparequisitosform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                formulario = TupaRequisitosForm(
                    request.POST or None, instance=tupa)
                if formulario.is_valid():
                    formulario.save()
                    return HttpResponseRedirect('/tupa/requisitos')
                return render(request, 'tuparequisitosform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
                ...
                if request.method == "POST":
                    ...
                    try:
                        if(len(id_r) > 0):
                            ...
                            for v in id_r:
                                id_requisito = v
                                requisitos = get_object_or_404(
                                    TupaRequisitos, pk=id_requisito)
                                # actualizando estado del expediente
                                vincular_obj = TupaVinculo(
                                    tupa=tupa, requisitos=requisitos)
                                vincular_obj.save()
                                # expediente_upd.estado="E"
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'tupavinculo.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'tupa': tupa, 'requisitos': requisitos})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'tupavinculo_detalle.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'tupas': tupas, 'requisitos': vinculo})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                tupa = get_object_or_404(Tupa, pk=id_tupa)
                requisitos = TupaRequisitos.objects.all()
                return render(request, 'tupavinculo_editar.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'tupa': tupa, 'requisitos': requisitos})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'proveido_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': tupa})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'proveidoform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'expedientetipo_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': tupa})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'expedientetipoform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                return render(request, 'organizacion_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': organizacion})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                formulario = OrganizacionForm(
                    request.POST or None, instance=organizacion)
                if formulario.is_valid():
                    formulario.save()
                    return HttpResponseRedirect('/organizacion')
                return render(request, 'organizacionform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                       'organizacion': empresa, 'Form': Lista}
                return render(request, 'TrabajadorControl.html', Ctx)
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
                ...
                if request.method == 'POST':
                    ...
                    if Formulario.is_valid():
                        try:
                            ...
                            Lista = Trabajador.objects.all()
                            Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                                   'organizacion': empresa, 'Info': Info, 'Form': Lista}
                            return render(request, 'TrabajadorControl.html', Ctx)
                        except:
                            Info = 'Error, El Dni Ingresado Ya Existe'
                            Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                                   'organizacion': empresa, 'Info': Info, 'Form': Formulario}
                            return render(request, 'TrabajadorNuevo.html', Ctx)
                    else:
                    ...
                    Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                           'organizacion': empresa, 'Form': Formulario, 'Info': Info}
                    return render(request, 'TrabajadorNuevo.html', Ctx)
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                       'organizacion': empresa, 'id_trabajador': id_trabajador, 'Trabajador': EliminarTrabajador}
                return render(request, 'TrabajadorEliminar.html', Ctx)
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                # if formulario.is_valid():
                 #   formulario.save()
                  #  return HttpResponseRedirect('/organizacion')
                # return render(request, 'organizacionform.html',{'NombreUsuario':NombreUsuario,'NombreOficina':NombreOficina,'organizacion':empresa,'formulario':formulario},context_instance=RequestContext(request))
                Info = 'Inicializando'
                Formulario = TrabajadorNuevoForm(
                    request.POST or None, instance=EditarTrabajador)
                if Formulario.is_valid():
                    try:
                        Formulario.save()
                        ...
                        return HttpResponse(html)
                return render(request, 'trabajadoreditar.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': Formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                try:
                    '''ALguna Restriccion'''
                    EliminarTrabajador = Trabajador.objects.get(
                        pk=id_trabajador)
                    EliminarTrabajador.delete()
                    Info = 'Se Elimino el trabajador: ' + \
                        EliminarTrabajador.nombre + ' ' + EliminarTrabajador.apellidos
                    Lista = Trabajador.objects.all()
                    Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                           'organizacion': empresa, 'Info': Info, 'Form': Lista}
                    return render(request, 'TrabajadorControl.html', Ctx)
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                ...
                Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                       'organizacion': empresa, 'id_oficina': id_oficina, 'Oficina': EliminarOficina}
                return render(request, 'OficinaEliminar.html', Ctx)
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                try:
                    ...
                    Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                           'organizacion': empresa, 'Form': Lista, 'Info': Info}
                    return render(request, 'OficinaControl.html', Ctx)
...
def oficina_editar(request, id_oficina):
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if UsuarioLogueado.admin == "S":
                Info = 'Inicializando'
                Formulario = OficinaForm(
                    request.POST or None, instance=EditarOficina)
                if Formulario.is_valid():
                    try:
                        Formulario.save()
                        ...
                        html = '<html><head><meta http-equiv="Refresh" content="3;url=/OficinaControl"></head><body>Error de parametro exception</body></html>'
                        return HttpResponse(html)
                return render(request, 'oficinaform.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'formulario': Formulario})
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        else:
                ...
                if request.method == 'POST':
                    ...
                    if Formulario.is_valid():
                        try:
                            ...
                            Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                                   'organizacion': empresa, 'Info': Info, 'Form': Formulario}
                            return render(request, 'OficinaNuevo.html', Ctx)
                    ...
                    Ctx = {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina,
                           'organizacion': empresa, 'Form': Formulario}
                    return render(request, 'OficinaNuevo.html', Ctx)
            else:
                html = '<html><head><meta http-equiv="Refresh" content="3;url=/"></head><body>Solo Administradores pueden ver esta opcion. <a href="/">Principal</a> </body></html>'
                return HttpResponse(html)
        ...
        if 'EstadoLogueo' in request.COOKIES:
            ...
            if request.method == 'POST':
                ...
                if Formulario.is_valid():
                    try:
                        UsuarioLogueado = Trabajador.objects.get(
                            usuario=request.POST['Usuario'], psw=request.POST['Contrasena'])
                        UsuarioOficina = Oficina.objects.get(
                            id=UsuarioLogueado.oficina_id)
                        ...
                        if UsuarioLogueado.admin == "S":
                            expediente = Expediente.objects.filter(
                                Q(estado="R") | Q(estado="C"))
                        response = render(request, 'expedientes.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'lista': expediente,
                                                                           'oficinas': oficinas, 'proveidos': proveidos, 'organizacion': empresa})
                        response.set_cookie(
                            'EstadoLogueo', 'Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ')
    ...
    if 'EstadoLogueo' in request.COOKIES:
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            try:
                ...
                NombreOficina = UsuarioOficina.nombre
                empresa = Organizacion.objects.get(id=1)
                return render(request, 'reportes.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa})
            except:
                NombreUsuario = ""
                NombreOficina = ""
                empresa = Organizacion.objects.get(id=1)
        ...
        if value == "Y3n1K0pSnlf3v7FdKRYaY5UdYvAfsYqZ":
            ...
            if request.GET.get("opcion"):
                ...
                if opcion == 1:
                    if intervalo == 1:
                        try:
                            titulo = "DOCUMENTOS RECIBIDOS HOY(" + \
                            ...
                            print(str(end_date))
                            recibidos = Derivar.objects.filter(Q(estado="R") | Q(
                                estado="C"), fecha__range=(start_date, end_date), oficina=UsuarioOficina)
                            return render(request, 'reportes_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': recibidos, 'titulo': titulo})
                        except Exception as e:
                            html = "<html><body> Excepcion <br>" + \
                                str(e) + "</body></html>"
                            return HttpResponse(html)
                    elif intervalo == 2:
                        titulo = "DOCUMENTOS RECIBIDOS ENTRE (" + \
                            fecha1 + " - " + fecha2 + ")"
                        start_date = datetime.strptime(fecha1, "%d/%m/%Y")
                        end_date = datetime.strptime(fecha2, "%d/%m/%Y")
                        recibidos = Derivar.objects.filter(estado="R").filter(
                            oficina=UsuarioOficina).filter(fecha__range=(start_date, end_date))
                        return render(request, 'reportes_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': recibidos, 'titulo': titulo})
                    else:
                    ...
                    if intervalo == 1:
                        titulo = "DOCUMENTOS DERIVADOS HOY(" + \
                            datetime.now().strftime("%d-%m-%Y") + ")"
                        start_date = datetime.now().strftime("%d/%m/%Y")
                        start_date = datetime.strptime(start_date, "%d/%m/%Y")
                        end_date = start_date + timedelta(days=1)
                        recibidos = Derivar.objects.filter(
                            estado="E", oficinao=UsuarioOficina, fecha__range=(start_date, end_date))
                        return render(request, 'reportes_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': recibidos, 'titulo': titulo})

                    elif intervalo == 2:
                        titulo = "DOCUMENTOS DERIVADOS ENTRE (" + \
                            fecha1 + " - " + fecha2 + ")"
                        start_date = datetime.strptime(fecha1, "%d/%m/%Y")
                        end_date = datetime.strptime(fecha2, "%d/%m/%Y")
                        recibidos = Derivar.objects.filter(estado="E").filter(
                            oficinao=UsuarioOficina).filter(fecha__range=(start_date, end_date))
                        return render(request, 'reportes_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': recibidos, 'titulo': titulo})
                    else:
                        html = "<html><body>Error de Intervalo</body></html>"
                        return HttpResponse(html)
                    ...
                    if intervalo == 1:
                        titulo = "DOCUMENTOS ARCHIVADOS HOY(" + \
                            datetime.now().strftime("%d/%m/%Y") + ")"
                        start_date = datetime.now().strftime("%d/%m/%Y")
                        start_date = datetime.strptime(start_date, "%d/%m/%Y")
                        end_date = start_date + timedelta(days=1)
                        recibidos = Derivar.objects.filter(estado="A").filter(
                            fecha__range=(start_date, end_date)).filter(oficina=UsuarioOficina)
                        return render(request, 'reportes_lista.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'lista': recibidos, 'titulo': titulo})

                    elif intervalo == 2:
                        titulo = "DOCUMENTOS ARCHIVADOS ENTRE (" + \
                            fecha1 + " - " + fecha2 + ")"
                        start_date = datetime.strptime(fecha1, "%d/%m/%Y")
                        end_date = datetime.strptime(fecha2, "%d/%m/%Y")
                        recibidos = Derivar.objects.filter(estado="A").filter(
                    ...
                    if intervalo == 1:
                        titulo = "MONTO ACUMULADO HOY(" + \
                            datetime.now().strftime("%d-%m-%Y") + ")"
                        start_date = datetime.now().strftime("%d-%m-%Y")
                        start_date = datetime.strptime(start_date, "%d-%m-%Y")
                        end_date = start_date + timedelta(days=1)
                        recibidos = Expediente.objects.filter(
                            f_registro__range=(start_date, end_date))
                        Suma = 0
                        for Item in recibidos:
                            Suma = Suma + Item.tupa.valor
                        return render(request, 'ReporteTotal.html', {'NombreUsuario': NombreUsuario, 'NombreOficina': NombreOficina, 'organizacion': empresa, 'Suma': Suma, 'titulo': titulo})

                    elif intervalo == 2:
                        titulo = "MONTO ACUMULADO ENTRE (" + \
                            fecha1 + " - " + fecha2 + ")"
                        start_date = datetime.strptime(fecha1, "%d/%m/%Y")
                        end_date = datetime.strptime(fecha2, "%d/%m/%Y")
                        recibidos = Expediente.objects.filter(
